...
The pickup version of this skill is niche, but has its uses especially when playing in stacks. It allows you to pickup an ally and transport them to wherever you fly. Allies being carried can attack, and will not dismount so long as it's not an ability that locks them in place like Ibara's alpha. You can als ocarry allies while they are drinking healing items or while they're lying on the ground after a knockdown (but does not work against DOWNd allies).
...
Gamma Skill: 20 DP (Per Consecutive Slash), 40 DP (All Tap Consecutive Slashes), 100 DP (All Hold Consecutive Slashes), 200 DP (Final Slash Level 7-9), 400 DP (Final Slash Level 1-5)<br>
Ground Melee: ?????????????????????????<br>
Air Melee: ??????????????????????<br>
<br>
